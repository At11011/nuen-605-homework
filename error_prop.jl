...
function problem1()
    ...
    println(
        "(iii) The Gaussian model is appropriate in this case since the mean is more than 25.",
    )
    ...
    bar(
        count,
        norm_prob,
        xtick = minimum(data):maximum(data),
        title = "(iv) Histogram of data",
        xaxis = "Counting measurement",
        yaxis = "Probability",
        label = "Data",
    )
    ...
    println(
        "The calculated mean ",
        calc_exp_mean == exp_mean ? "does" : "does not",
        " match the experimental mean.",
    )
    println(
        "(vi) The data seems to roughly match a Gaussian distribution, but it is a very rough fit. ",
...
function problem2()
    ...
    println("\nProblem 2:")
    println(
        "(i) In this case, since the mean is greater than 25, the Gaussian model is best.",
    )
    println(
        "(ii) When considering each measurement individually, the standard deviation can be calculated by taking" *
    ...
    [
        println(
            "\t$(@sprintf("%d", Measurements.value(val))) ± $(@sprintf("%1.2f", Measurements.uncertainty(val))) counts",
        ) for val in totals
    ]
    ...
    [
        println(
            "\t$(@sprintf("%d", Measurements.value(ustrip(val)))) ± $(@sprintf("%1.2f", Measurements.uncertainty(ustrip(val)))) counts/s",
        ) for val in count_rate
    ]
...
function problem3()
    ...
    function error_prop(u, subx, suby, subz)
        ...
        println(
            "\tdu²dx²:\t\t$(expand_derivatives(substitute(du²dx², Dict([x => subx, y => suby, z => subz]))))",
        )
        println(
            "\tdu²dy²:\t\t$(expand_derivatives(substitute(du²dy², Dict([x => subx, y => suby, z => subz]))))",
        )
        println(
            "\tdu²dz²:\t\t$(expand_derivatives(substitute(du²dz², Dict([x => subx, y => suby, z => subz]))))",
        )
        return du²dx² * σx^2 + du²dy² * σy^2 + du²dz² * σz^2
    end
    ...
    println("\tError Equation:\tσA = √($(substitute(expand_derivatives(σᵤ), Dict([x => A₀, y => λ, z => t]))))")
...
function problem4()
    ...
    println("\nProblem 4")
    println(
        "(i) The inverse square law states that the intensity of a source is inversely proportional to the square of the distance from the source.",
    )
...
function problem5()
    ...
    println("\nProblem 5:")
    println(
        "(i) In this case, the experimental mean is unknown, and the success/failure is determined by a successful or unsuccessful measurement. This type of binary criteria lends itself to the binomial distribution.",
    )
    part_prob =
        factorial(big(total), big(total - partial)) / factorial(big(partial)) *
        prob^partial *
        (1 - prob)^(total - partial)
    println("(ii) The probability is $(@sprintf "%0.5f" part_prob)")
    avg_success = prob * total
    std_dev = sqrt(avg_success * (1 - prob))
    println(
        "(iii) The predicted average number of successes is $(@sprintf "%0.1f. The standard deviation is %0.5f." avg_success std_dev)",
    )
end

function problem6()
    time = 60u"s"
    count_back = 81 ± sqrt(81)
    count_total = 567 ± sqrt(567)
    count_source = count_total - count_back
    println("\nProblem 6:")
    println("(i) Net count:\t$count_source")
    println(
        "(ii) Count rates:\n\tBackground:\t$(ustrip(count_back / time)) counts/s\n\tWith source:\t $(ustrip(count_total/time)) counts/s",
    )
    # count_back_unc = sqrt(count_back)
    # count_total_unc = sqrt(count_total)
    # count_source_unc = sqrt(count_back_unc^2 + count_total_unc^2)
    println(
        "(iii) Net Count Rate:\t$((count_source) / ustrip(time)) counts/s",
    )
end
...
function problem7()
    ...
    println("\nProblem 7:")
    println(
        "(i) $count counts are needed for $(@sprintf "%0.3f" frac_std)% fractional standard deviation.",
    )
end
...
function problem8()
    ...
    # expr = (x + y) / 2
    println("\nProblem 8:")
    println("(i) Average and Error:\t$(average)")
end
